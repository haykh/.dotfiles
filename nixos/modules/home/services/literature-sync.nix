...
let
  this = config.my.services.literature-sync;
  home = cfg.home;
...
in
{
  ...
  options.my.services.literature-sync.enable =
    lib.mkEnableOption "bidirectional sync of ${remote} <-> ${localDir}";

  config = lib.mkIf this.enable {

    systemd.user.services.literature-sync = {
